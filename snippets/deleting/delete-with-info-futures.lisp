;;;; delete-with-info-futures.lisp — Concurrent deletion with WITH_INFO responses
;;;
;;; Usage:
;;;   sbcl --non-interactive --load snippets/deleting/delete-with-info-futures.lisp
...
(defpackage #:delete-with-info-futures
  (:use #:cl #:sz-sdk #:sz-sdk.flags))

(in-package #:delete-with-info-futures)

(defparameter *instance-name* "delete-with-info-futures")
...
(defparameter *input-file*
  (merge-pathnames "data/del-500.jsonl" *resources-dir*))

(defparameter *output-file*
  (merge-pathnames "output/delete_file_with_info.jsonl" *resources-dir*))
...
(defun delete-record-with-info (engine record-line)
  (let* ((record (yason:parse record-line))
         (data-source (gethash "DATA_SOURCE" record ""))
         (record-id (gethash "RECORD_ID" record "")))
    (delete-record engine data-source record-id :flags +sz-with-info+)))

(defun futures-del (engine input-file output-file)
  (let ((error-recs 0)
        (success-recs 0)
             ...
             (let ((futures (make-hash-table :test 'eq)))
               (loop repeat num-workers
                     ...
                     do (let ((l line))
                          (setf (gethash
                                 (lparallel:future
                                   (delete-record-with-info engine l))
                                 futures)
                                l)))
...
(handler-case
    (with-sz-factory (factory *instance-name* *settings*)
      (let ((engine (create-engine factory)))
        (futures-del engine *input-file* *output-file*)))
  (sz-error (e)
